...
def CATS(content):
	...
	md.addDir({'mode': '7', 'name':'[B][COLOR white]SEARCH[/COLOR][/B]', 'url':'url', 'content':content})
...
def INDEX(url,content):
	...
	for a in all_videos:
		...
		if content == 'movies':
                        if qual:
                                md.addDir({'mode': '8', 'name':'[B][COLOR white]%s[/COLOR][I][COLOR red](%s)[/COLOR][/I][/B]' %(name,qual),
...
def EPIS(title, url, iconimage, content, season):
	...
	for a in all_videos:
		...
		fan_art = {'icon':iconimage}
                ...
                md.addDir({'mode': '9', 'name':'[B][COLOR white]%s[/COLOR][/B]' %name,
	...
	try:
		...
		for a in all_videos:
			...
			res_quality = []
                        ...
                        if auto_play == 'true':
                                url = max(final['playlist'][0]['sources'], key=lambda lab: lab['label'])
                                url = url['file']
                                for a in final['playlist'][0]['sources']:
                                        if '1080p' in a['label']:
                                                url = a['file']
                        else:
